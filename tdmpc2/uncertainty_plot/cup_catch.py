...
def evaluate_world_model_landscape(cfg: dict):
    """Loads the TD-MPC2 agent and evaluates density and variance across a 2D grid of ball positions."""
    cfg['task'] = 'cup-catch'
    cfg = parse_cfg(cfg)
    env = make_env(cfg)
    agent = TDMPC2(cfg)
    ...
    surf0 = ax0.plot_surface(X, Z, density_2d, cmap='viridis', edgecolor='none', alpha=0.9)
    ...
    surf1 = ax1.plot_surface(X, Z, var_2d, cmap='plasma', edgecolor='none', alpha=0.9)
    ...
    return X, Z, reward_2d, value_2d, y
